...
const updateHostType = `-- name: UpdateHostType :exec
UPDATE
	hosts
SET
	type = ?1
...
type UpdateHostTypeParams struct {
	Type    *string
	Host    string
	Zone    string
	Cluster string
...
//			OR (cluster = (SELECT id FROM clusters c WHERE c.name = ?4 AND zone = (SELECT id FROM zones z WHERE z.name = ?3)))
//		)
func (q *Queries) UpdateHostType(ctx context.Context, arg UpdateHostTypeParams) error {
	_, err := q.db.ExecContext(ctx, updateHostType,
		arg.Type,
		arg.Host,
		arg.Zone,
		arg.Cluster,
